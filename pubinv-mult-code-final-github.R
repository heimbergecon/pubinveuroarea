...
dt <- subset(dt, year %in% c('2000', '2001', '2002', '2003', '2004', '2005', '2006',' 2007', '2008', '2009', '2010', '2011', '2012', '2013', '2014', '2015', '2016', '2017', '2018', '2019', '2020', '2021', '2022', '2023'))
...
# standard error bands - symmetrical one standard error band
...
lp_gdp <- lp_lin_panel(
  ...
  robust_cov     = "vcovSCC",
  l_exog_data    = c("growth_RGDP","PDEBT","forecasterror","NOMLRATE", "REER"),
  lags_exog_data = 2,
  confint        = 1,
  hor            = 4
)
...
lp_priv <- lp_lin_panel(
  ...
  panel_effect   = "twoways",
  robust_cov     = "vcovSCC",
  l_exog_data    = c("growth_RGDP","PDEBT","forecasterror","NOMLRATE","INVGDP_diff", "REER"),
  lags_exog_data = 2,
  confint        = 1,
  hor            = 4
)
...
lp_unemp <- lp_lin_panel(
  ...
  panel_effect   = "twoways",
  robust_cov     = "vcovSCC",
  l_exog_data    = c("growth_RGDP","PDEBT","forecasterror","NOMLRATE","UNRATE", "REER"),
  lags_exog_data = 2,
  confint        = 1,
  hor            = 4
)
...
ctrl_rob1 <- c("OUTPUTGAP","PDEBT","forecasterror","NOMLRATE", "REER")
mult_rob1 <- run_mult(ctrl_rob1, lags = 2)
print(mult_rob1)

## Robustness 2: increase lag length to 3 (baseline controls)
ctrl_base <- c("growth_RGDP","PDEBT","forecasterror","NOMLRATE", "REER")
mult_rob2 <- run_mult(ctrl_base, lags = 3)
print(mult_rob2)

## Robustness 3: add PRIMARYBAL to baseline controls
ctrl_rob3 <- c("growth_RGDP","PDEBT","forecasterror","NOMLRATE","PRIMARYBAL", "REER")
mult_rob3 <- run_mult(ctrl_rob3, lags = 2)
print(mult_rob3)

## Robustness 4: exclude Ireland (ccode == "IRL")
...
## Robustness 5: stop sample in 2019 (pre-Covid)
dt_pre_covid <- subset(dt, as.integer(year) <= 2019)

rbar_pre_covid <- mean(dt_pre_covid$PUBINVRATIO, na.rm = TRUE)
if (!is.na(rbar_pre_covid) && rbar_pre_covid > 1) rbar_pre_covid <- rbar_pre_covid / 100
